...
const realmTemplate = require('../config/keycloak.realm-template.json');

realmTemplate.id = "Nuf"
realmTemplate.realm = "Nuf"
const userProvider = realmTemplate.components['org.keycloak.storage.UserStorageProvider'].find(k => k.name == "jdbc-user-provider");
console.log("userProvider", userProvider);
userProvider["config"]["dbPassword"] = ["app_p"];

// The public and bearer clients that will be set up
const PUBLIC_CLIENT_NAME = `${realmTemplate.realm}-public`
const BEARER_CLIENT_NAME = `${realmTemplate.realm}-bearer`
...
async function initKeycloak() {
    ...
    await kc.realms.del({ realm: realmTemplate.realm }).catch((err) => {
        if (err.response.status !== 404) {
            throw err // if we get a 404 that's fine, if we get something else, throw it.
        }
    })

    let result = await kc.realms.create(realmTemplate)

    kc.setConfig({
        realmName: realmTemplate.realm,
    })
